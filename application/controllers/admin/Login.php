	...
	public function userlogin()
{
    ...
    else
    {
        ...
        // Check if data is an array AND it has at least one element
        if(is_array($data) && !empty($data))
        {
            $loginName = $data[0]['Name'];
            $company_id = $data[0]['company_id']; 
            $loginusername = $data[0]['UserName'];
            $loginid = $data[0]['userid'];
            $roleid = $data[0]['userroleid'];
            
            $this->session->set_userdata('loginName', $loginName);
            $this->session->set_userdata('logged_in_company_id', $company_id);
	...
	// 		{
	// 			$loginName=$data[0]['Name'];
	// 			$company_id=$data[0]['company_id']; //login user store id
	...
	// 				setcookie('adminremember',$_POST["remember"], $hour);
	// 			}
	// 			if($data[0]['userroleid']==1){   //if login user is admin redirected to admin dashboard otherwise owner dashboard
	// 				redirect('admin/enquiry');
	// 			}else{
	// 				redirect('admin/enquiry');
	// 			}
...
{
    $this->db->where('user_id', $this->session->userdata('loginid'))
             ->where('company_id', $this->session->userdata('logged_in_company_id'))
             ->set('logout_time', date('Y-m-d h:i:s A')) // set the field and value
             ->update('user_activity'); // specify the table
